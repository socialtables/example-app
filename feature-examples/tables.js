...
function getNextEventWithTables(config, teamID, legacyUserID, oauthToken) {
	...
	return request({
		method: "GET",
		headers: authHeaders,
		uri: `${config.socialtables_api_host}/4.0/legacyvm3/users/${legacyUserID}/events`,
		qs: {
	...
	.then(function() {
		if (event) {
			return request({
				method: "GET",
				headers: authHeaders,
				uri: `${config.socialtables_api_host}/4.0/legacyvm3/teams/${teamID}/events/${event.id}/elements`,
				json: true
			});
		}
	})
